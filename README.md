# Front-end
l'app utilise le fichier json actuellement.

pour la faire fonctionner le back-end : dans les deux composants products-admin, products et dans le service product.
- il faut commenter les parties du code qui sont sous le commentaire suivant :
   /**
   * Utilisation avec le fichier Json
   */
- décommenter les partie du code qui sont sous le commentaire suivant :
   /**
   * Utilisation avec le back-end
   */

# back-end
Dans le fichier application.properties, modifier les données d'accées à une base de données.
- spring.datasource.url=jdbc:postgresql://localhost:5432/BDD_Test
- spring.datasource.username=
- spring.datasource.password=
- spring.datasource.driver-class-name=org.postgresql.Driver
Ensuite lancer l'application, spring va générer automatiquement la table en BDD. ce qui permet de tester le front et le back.
